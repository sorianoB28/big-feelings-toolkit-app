...
const allowedAvatarKeys = new Set<string>(AVATARS.map((avatar) => avatar.key));
const allowedThemeKeys = new Set<string>(THEMES.map((theme) => theme.key));
...
function parseStudentInput(formData: FormData): StudentMutationInput {
  ...
  const avatarInput = normalizeText(formData.get("avatar_key"));
  const themeInput = normalizeText(formData.get("theme_key"));
  ...
  if (avatarInput && !allowedAvatarKeys.has(avatarInput)) {
    throw new Error("Avatar selection is invalid.");
  }
  if (themeInput && !allowedThemeKeys.has(themeInput)) {
  ...
  return {
    ...
    avatarKey: avatarInput || null,
    themeKey: themeInput || null,
    active,
  };
}
...
export async function createStudentAction(formData: FormData): Promise<void> {
  ...
  try {
    ...
    redirectPath = `/students/${created.id}?created=1`;
  } catch (error) {
...
export async function updateStudentAction(studentId: string, formData: FormData): Promise<void> {
  ...
  try {
    ...
    redirectPath = `/students/${studentId}?updated=1`;
  } catch (error) {
